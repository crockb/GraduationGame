...
public class PlaceTower : MonoBehaviour
{
	...
	public GameObject starbucksPrefab;
	private GameObject starbucks;
	...
	private bool CanPlaceStarbucks()
	{
  		int cost = starbucksPrefab.GetComponent<StarbucksData>().levels[0].cost;
		return starbucks == null && gameManager.GraduationCoins >= cost;
	}
	...
	private bool CanUpgradeStarbucks()
	{
	  ...
	  {
	    StarbucksData starbucksData = starbucks.GetComponent<StarbucksData>();
	    StarbucksLevel nextLevel = starbucksData.GetNextLevel();
	    if (nextLevel != null)
	    {
			return gameManager.GraduationCoins >= nextLevel.cost;
	    }
	  }
	  return false;
	}
...
// SECONDARY WORKING CODE

public class PlaceTower : MonoBehaviour
{

	public GameObject starbucksPrefab, libraryPrefab;
	private GameObject starbucks, library;
	private string activeTowerLabel;
	private int cost;
	...
	void OnMouseUp()
	{
		...
		if (activeTowerLabel == "None")
		{
				// do nothing
		}
		else if (activeTowerLabel == "PlaceStarbucks")
		{
			if (CanPlaceStarbucks())
	  		{
	    		//3
		...
		else if (activeTowerLabel == "PlaceLibrary")
		{
			if (CanPlaceLibrary())
	  		{
	    		//3
	...
	{
  		cost = starbucksPrefab.GetComponent<StarbucksData>().levels[0].cost;
		return starbucks == null && library == null && gameManager.GraduationCoins >= cost;
	}
	...
	{
  		cost = libraryPrefab.GetComponent<LibraryData>().levels[0].cost;
		return library == null && starbucks == null && gameManager.GraduationCoins >= cost;
	}
